 ...
 */
const INSERTED_BLOCK = String.raw`/* dsh-model-extended:per-model-declarations */
...
/**
 * Write the patch into a bundle's text: the disclosure gains the two extra
 * fields, and the renderers are declared beside the official `capacityField`.
...
export function patchSource(source) {
	return source.replace(ANCHOR_CHILDREN, CHILDREN_PATCHED).replace(ANCHOR_DEFINITION, INSERTED);
}

/**
 * Remove the patch from a bundle's text. Pure counterpart of
 * {@link patchSource}: the injected block goes back to the anchor it was
 * inserted before, so the official `capacityField` definition survives.
 *
 * @param source - bundle text.
 * @returns the pristine text.
 */
export function unpatchSource(source) {
	return source.replace(INSERTED, ANCHOR_DEFINITION).replace(CHILDREN_PATCHED, ANCHOR_CHILDREN);
}
