...
function buildResolver(ctx, namespaces) {
	// A strict opt-in list. It must not widen to "every namespace" when empty:
	// pi-ai declares these fields itself, and overlaying its route would replace
	// the adapter's own answer with this plugin's reading of the same value.
	const allowed = new Set(namespaces);
	return (provider, modelId) => {
		if (typeof modelId !== "string") return undefined;
		const ns = namespaceForProvider(ctx, provider);
		if (ns === undefined || !allowed.has(ns)) return undefined;
