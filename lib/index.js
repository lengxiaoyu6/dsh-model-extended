...
const log = (ctx, ...args) => {
	try {
		ctx.logger?.info?.("[model-set]", ...args);
	} catch {
...
function wrapAdapter(ctx, adapter, resolveEntry) {
	if (adapter === null || typeof adapter !== "object" || adapter.__modelSetWrapped === true) return;
	...
	if (wrapped) adapter.__modelSetWrapped = true;
}

/**
 * Collect every declared model entry by settings namespace, straight out of the
 * resolved settings values. Schemastery preserves unknown keys, so this plugin's
 * own fields arrive exactly as the patched editor stored them.
 *
 * @param ctx - plugin context.
 * @param namespaces - namespaces to read.
 * @returns per-namespace maps from model id to its declaration-carrying entry.
 */
function collectDeclarations(ctx, namespaces) {
	const settings = ctx.get("settings");
	const collected = new Map();
	if (settings === undefined || namespaces.length === 0) return collected;
	for (const ns of namespaces) {
		...
		collected.set(ns, byId);
	}
	return collected;
}

/**
 * Map each registered provider route to the settings namespace that configures
 * it, then to its declared model entries. Route→namespace comes from the
...
function buildResolver(ctx, namespaces) {
	const collected = collectDeclarations(ctx, namespaces);
	const nsByProvider = new Map();
	try {
		for (const entry of ctx.llm.listConfigurableProviders()) {
			if (typeof entry?.provider === "string" && typeof entry?.settingsNs === "string") {
				nsByProvider.set(entry.provider, entry.settingsNs);
			}
		}
	} catch {
		/* a directory read must not disable the overlay for routes already known */
	}
	return (provider, modelId) => {
		if (typeof modelId !== "string") return undefined;
		const ns = nsByProvider.get(provider);
		if (ns === undefined) return undefined;
		return collected.get(ns)?.get(modelId);
	};
}
